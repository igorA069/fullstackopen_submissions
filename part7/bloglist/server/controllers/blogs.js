...
blogRouter.delete("/blogs/:id", userExtractor, async (request, response) => {
  ...
  if (!userFromTokenInDb) {
    return response
      .status(403)
      .json({ error: "token references unknown user" });
  }
  ...
  if (userIdFromBlog?.toString() !== userFromTokenInDb._id.toString()) {
    return response.status(403).json({
      error: "user requesting deletion must be the creator of the resource",
    });
  }
...
blogRouter.post(
  ...
  async (request, response) => {
    const blogToUpdate = await Blog.findById(request.params.id);
    blogToUpdate.comments = blogToUpdate.comments.concat(request.body.content);
    await blogToUpdate.save();
    response.status(200).end();
  },
);
